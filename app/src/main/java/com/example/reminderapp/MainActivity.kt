    ...
    private fun createItemTouchHelper(): ItemTouchHelper{
        ...
        val callback = object :  ItemTouchHelper.SimpleCallback(0,ItemTouchHelper.LEFT){
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                //start a coroutine (sort of lightweight thread) to async perform the deletion of the reminder.
                CoroutineScope(Dispatchers.Main).launch {
                    withContext(Dispatchers.IO) {
                        reminderRepository.deleteReminder(reminderToRemove)
                    }
                    getRemindersFromDatabase()
                }
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            when (requestCode) {
                ...
                ADD_REMINDER_REQUEST_CODE -> {
                    ...
                    //perform the insert on a coroutine to perform it async 
                    CoroutineScope(Dispatchers.Main).launch {
                        withContext(Dispatchers.IO) {
                            //insert the reminder into the database using the repo and fetch the reminders again
